...
const USER = require("/etc/vigibot/robot.json");
...
const SP = require("./rdk-serialport");
const GPIO = require("./rdk-pigpio").Gpio;
const I2C = require("./rdk-i2c-bus");
const PCA9685 = require("./rdk-pca9685");
const GPS = require("gps");
const INA219 = require("./rdk-ina219");
...
USER.SERVERS.forEach(function(server, index) {
 ...
 sockets[server].on("clientsrobottx", function(data) {
  ...
  if(data.boucleVideoCommande && data.boucleVideoCommande > 1e12)
...
setInterval(function() {
 ...
 if(predictiveLatency < SYS.LATENCYALARMEND && latencyAlarm) {
  ...
  latencyAlarm = false;
 } else if(false && predictiveLatency > SYS.LATENCYALARMBEGIN && !latencyAlarm) {
